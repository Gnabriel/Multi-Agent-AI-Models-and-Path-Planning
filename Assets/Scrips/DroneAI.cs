...
public class DroneAI : MonoBehaviour
{
    ...
    float steer_k = 15f;//max distance units for RRT (seems like higher values result in smoother paths)
    ...
    float limitation = 0.75f;//fraction of max_spd or acc allowed (right now it only affects speed)
    float goal_thresh = 0.2f;//how many units away from the goal can be considered a success
    int max_iters = 10000;//max iterations or nodes for RRT/RRT*

    //PARAMS OF BEST RESULTS SO FAR: 
    //A: goal_thresh = 0.5; limitation = 0.5; collision = 3.5; steer = 8; plan_steer = 20; optimal k = 1; max iters = 20K. Time ~ 70s
    //B: goal_thresh = 0.5; limitation = 0.5; collision = 3.5; steer = 8; plan_steer = 20; optimal k = 1; max iters = 25K. Time ~ 124s
    //C: goal_thresh = 0.2; limitation = 0.75; collision = 3.5; steer = 15; plan_steer = 20; optimal k = 1; max iters = 10K. Time ~ 12s
